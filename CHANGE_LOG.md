...
## 2023-07-XX - 画像生成機能の改善

### 変更内容
1. **混乱検出機能の削除**
   - ユーザーの混乱を自動検出して画像生成を提案する機能を削除
   - `detectConfusionWithLLM` 関数を簡略化し、常に false を返すように変更

2. **明示的な画像生成リクエストのみに対応**
   - ユーザーが「画像を生成して」などと明示的に依頼した場合のみ画像生成を行うように変更
   - 新しい `isImageGenerationRequest` 関数を追加して画像生成リクエストを検出

3. **システムプロンプトの更新**
   - AIに「ユーザーが理解できない場合は、画像生成で説明できます」と伝えるよう指示
   - ユーザー向けマニュアルを更新し、画像生成の使い方を明確化

4. **アプリの趣旨に反するリクエストの拒否**
   - 画像生成リクエストがアプリの趣旨に反する場合、丁寧にお断りする機能を追加
   - `securityFilterPrompt` を使用して内容をチェック

### 技術的な変更点
1. `isConfusionRequest` 関数を修正し、`isImageGenerationRequest` 関数を呼び出すように変更
2. 混乱検出のLLM分析部分を削除
3. 画像生成リクエスト処理を直接的なリクエストのみに対応するよう変更
4. システムプロンプト（`SYSTEM_PROMPT_GENERAL`）を更新

### 影響範囲
- ユーザーは混乱検出による自動的な画像生成提案を受け取らなくなります
- 画像生成を利用するには、明示的に「画像を生成して」などと依頼する必要があります
- AIは理解が難しい場合に画像生成機能を案内するようになります
- アプリの趣旨に反する画像生成リクエストは丁寧にお断りされます