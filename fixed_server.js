...
さらに、あなたには画像認識と限定的な画像生成の機能が備わっています：

【画像認識】送信された画像ファイルを解析・説明できます。
【画像生成】特定のキーワードを含む明示的なリクエストのみ対応可能です。

**重要な制限事項**：
- 画像生成は「画像を生成して」「イメージを作って」「○○で生成してください」などの明確な生成指示語が含まれる場合のみ実行されます
- 曖昧な表現や間接的な依頼では画像生成はできません
- 生成できない場合は「申し訳ありませんが、そのリクエストでは画像生成機能が作動しません。『○○の画像を生成してください』のような明確な指示をお試しください」と正直に回答してください
- 実際にできないことを「やります」「進めています」と言うのは禁止です
...
・画像生成をご希望の場合は、「〇〇の画像を生成してください」「〇〇のイメージを作って」など、明確な生成指示を含めてお願いしてください。曖昧な表現では機能が作動しません。
・音声入力機能もご利用いただけます（1日3回まで）。サービス向上のため、高いご利用状況により一時的にご利用いただけない場合もございますので、あらかじめご了承ください。順次改善するようにします。
・あなたの基本機能は、「適職診断」「特性分析」「画像解析」のほか、明確な指示がある場合の限定的な画像生成もできます。
`;
...
function isDirectImageGenerationRequest(text) {
  ...
  const imageGenerationRequests = [
    ...
    // 髪型・ヘアスタイル関連の生成リクエスト
    'で生成してください', 'で生成して', 'を生成してください', 'を生成して', 
    'の画像', 'のイメージ', 'のスタイル', 'のヘアスタイル', 'の髪型',
    'ヘアスタイルを', '髪型を', 'マッシュで', 'ボブで', 'ショートで', 'ロングで'
  ];
...
【重要な制約】
- 直前のあなたの回答（【直前のあなたの回答】セクションに記載される場合）を必ず参照する
- ユーザーの「それ」「その」「この」などの指示語は直前の回答を指している
...
文脈が不明な場合は「どの部分について詳しく知りたいですか？」と確認してください。`;
    default:
...
${additionalPromptData.jobTrends.analysis}`;
    ...
    // 2.4.5 Add Deep Exploration specific enhancement
    if (mode === 'deep-exploration' && history && history.length > 0) {
      console.log('\n🔍 [2D] ADDING DEEP EXPLORATION CONTEXT');
      const deepExplorationStartTime = Date.now();
      
      // 直前のAI回答を特定（最新のassistantメッセージ）
...
「それ」「その部分」「この内容」などの指示語は、この直前の回答の内容を指していると解釈してください。`;
        
        console.log(`🔍 [2D] Added previous AI response (${lastAiResponse.content.length} chars) for deep exploration context`);
      } else {
        console.log(`⚠️ [2D] No previous AI response found for deep exploration mode`);
      }
      
      console.log(`🔍 [2D] DEEP EXPLORATION CONTEXT - Completed in ${Date.now() - deepExplorationStartTime}ms`);
    }
