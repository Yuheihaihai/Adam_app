...
async function processWithAI(systemPrompt, userMessage, historyData, mode, userId, client) {
  try {
    ...
    // モデル選択リクエストの処理
    if (specialCommands.hasModelSelection && specialCommands.modelQuery) {
      console.log(`\n🤖 [モデル選択] ${specialCommands.selectedModel}モードで実行: "${specialCommands.modelQuery}"`);
      
      try {
        // 選択されたモデルに基づいてOpenAIモデル名を設定
        let modelName = 'gpt-4o';
        
        if (specialCommands.selectedModel.includes('gpt-4o') || specialCommands.selectedModel.includes('4o')) {
          modelName = 'gpt-4o';
        } else if (specialCommands.selectedModel.includes('gpt-4') || specialCommands.selectedModel === 'gpt-4') {
          modelName = 'gpt-4-turbo';
        } else if (specialCommands.selectedModel.includes('gpt-3.5') || specialCommands.selectedModel.includes('3.5')) {
          modelName = 'gpt-3.5-turbo';
        }
        
        console.log(`\n🤖 [モデル選択] ${modelName}を使用します`);
        ...
        const messages = [
          { role: 'system', content: systemPrompt },
          ...userMessages,
          { role: 'user', content: specialCommands.modelQuery }
        ];
        ...
        console.log(`\n✅ [モデル選択] ${modelName}応答生成完了: ${responseContent.length}文字`);
        
        // 応答をデータベースに保存
        const assistantMessage = { 
          role: 'assistant', 
          content: `🤖 [${modelName}] ${responseContent}`
        };
