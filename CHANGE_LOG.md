...
## 2025-03-31: キャリア分析機能の強化

### 適職診断と特性分析の区別を改善

**変更内容:**
- キャリア関連クエリの検出ロジックを`enhancedCharacteristicsAnalyzer.js`に追加
- キャリア分析専用のプロンプトを実装し、具体的な職業推奨を含む詳細な分析を提供
- OpenAIフォールバック時にもキャリアモードが適切に処理されるよう修正
- `generateCareerAnalysis`関数の新規実装

**変更理由:**
ユーザーが「適職診断」や「キャリア分析」を要求した際に、一般的な特性分析ではなく、具体的な職業推奨や職場環境の提案を含む詳細なキャリア分析を提供できるよう改善しました。この変更により、適職診断リクエスト時には必ず具体的な職業名や働き方の推奨が含まれるようになり、ユーザー体験が向上します。さらに、Gemini APIが使用できない場合のOpenAIフォールバック時にも、同様にキャリア特化型の回答を提供することで一貫性を確保しています。
