...
async function processMessage(userId, messageText) {
  try {
    ...
    // メッセージからモードを検出
    const { mode, limit } = determineModeAndLimit(messageText);
    console.log(`📊 モード検出: "${messageText.substring(0, 30)}..." => モード: ${mode}, 履歴制限: ${limit}件`);
    ...
    // 特性分析や適職診断モードの場合はAirtableから履歴を取得
    let airtableHistory = [];
    if (mode === 'characteristics' || mode === 'career' || mode === 'memoryRecall') {
      console.log(`📊 ${mode}モードを検出: Airtableから過去${limit}件の履歴を取得します`);
      try {
        const historyData = await fetchUserHistory(userId, limit);
        if (historyData && historyData.history && historyData.history.length > 0) {
          console.log(`📊 Airtableから${historyData.history.length}件の履歴を取得しました`);
          airtableHistory = historyData.history;
        } else {
          console.log(`📊 Airtableからの履歴取得: データなし`);
        }
      } catch (historyErr) {
        console.error(`📊 Airtableからの履歴取得エラー:`, historyErr);
      }
    }
    
    // 会話履歴の取得（セッションからの履歴またはAirtableからの履歴を使用）
    const history = (mode === 'characteristics' || mode === 'career' || mode === 'memoryRecall') && airtableHistory.length > 0 
                    ? airtableHistory 
                    : (sessions[userId].history || []);
    
    console.log(`📊 会話処理に使用する履歴: ${history.length}件 (モード: ${mode})`);
    
    // GPT-4oモデルを使用して応答を生成
    const systemPrompt = getSystemPromptForMode(mode);
    console.log(`📊 システムプロンプト: ${mode}モード, ${systemPrompt.substring(0, 50)}...`);
    
    // 特性分析モードの場合は特別な処理を行う
    if (mode === 'characteristics') {
      console.log(`📊 特性分析モード: 過去の会話履歴から特性分析を行います (${history.length}件)`);
      const analysisResult = await generateHistoryResponse(history);
      
      // 会話履歴を更新
      sessions[userId].history.push({ role: "user", content: messageText });
      sessions[userId].history.push({ role: "assistant", content: analysisResult });
    ...
    const response = await generateAIResponse(messageText, history, contextMessages, userId, mode, systemPrompt);
