...
app.post('/webhook', rawBodyParser, (req, res) => {
  ...
  try {
    ...
    const stringBody = Buffer.isBuffer(rawBody) ? rawBody.toString() : JSON.stringify(rawBody);
    const isValid = validateSignature(stringBody, config.channelSecret, signature);
  ...
  console.log('Webhook was called! Events:', JSON.stringify(parsedBody, null, 2));
  
  // リクエストにeventsがない場合のエラー処理を追加
  if (!parsedBody || !parsedBody.events || !Array.isArray(parsedBody.events)) {
    console.warn('Invalid webhook request format:', parsedBody);
  ...
  (async () => {
    try {
      // 各イベントを非同期で処理
      const results = await Promise.all(parsedBody.events.map(event => {
...
// (ここまでが置き換え後のコード)
    ...
    // 履歴の取得
    console.log(`📚 テストエンドポイント: 会話履歴取得プロセス開始 - ユーザー: ${userId}`);
    const historyData = await fetchUserHistory(userId, limit) || [];
    const history = Array.isArray(historyData) ? historyData : (historyData.history || []);
    console.log(`📚 テストエンドポイント: 会話履歴取得完了: ${history.length}件`);
    ...
    if (semanticSearch && typeof semanticSearch.findSimilarMessages === "function") {
      try {
        ...
        console.error("テストエンドポイント: セマンティック検索エラー:", searchErr);
      }
    }
    ...
    // 特性分析モードの場合の特別処理
    if (mode === "characteristics") {
      console.log("テストエンドポイント: 特性分析モードを開始します");
      try {
        ...
        console.error("テストエンドポイント: 特性分析処理エラー:", err);
        response = "申し訳ありません、特性分析中にエラーが発生しました。";
      }
    }
    // 適職診断モードの場合の特別処理
    else if (mode === "career") {
      console.log("テストエンドポイント: 適職診断モードを開始します");
      // キャリア分析専用の関数を呼び出し
      try {
        response = await generateCareerAnalysis(history, text);
      } catch (err) {
        console.error("テストエンドポイント: キャリア分析エラー:", err);
        response = "申し訳ありません、キャリア分析中にエラーが発生しました。";
      }
    }
    // 通常の会話応答の生成
    else {
      try {
        ...
        console.error("テストエンドポイント: AI応答生成エラー:", err);
        response = "申し訳ありません、応答生成中にエラーが発生しました。";
      }
    }
    ...
    try {
      ...
      console.error("テストエンドポイント: 会話保存エラー:", storageErr);
    }
    ...
    res.status(500).json({ error: "内部サーバーエラー", message: error.message });
  }
});
...
// DISABLED:       
// DISABLED:       // ASD支援ガイドテキストを定義
// DISABLED:       const ASDSupportManual = `
// DISABLED: 【ASD支援機能の使い方ガイド】
// DISABLED: 
// DISABLED: Adamでは以下のようなASD(自閉症スペクトラム障害)に関する質問や相談に対応できます：
// DISABLED: 
// DISABLED: ■ 対応可能な質問例
// DISABLED: • 「自閉症スペクトラムの特性について教えて」
// DISABLED: • 「ASDの子どもとのコミュニケーション方法は？」
// DISABLED: • 「感覚過敏への対処法を知りたい」
// DISABLED: • 「社会的場面での不安に対するアドバイスが欲しい」
// DISABLED: • 「特定の興味や関心を活かせる仕事は？」
// DISABLED: • 「構造化や視覚支援の方法を教えて」
// DISABLED: • 「学校や職場での合理的配慮について」
// DISABLED: 
// DISABLED: ■ 使い方
// DISABLED: • テキストで質問するだけ：気になることを自然な言葉で入力してください
// DISABLED: • 継続的な会話：フォローアップ質問も自然にできます
// DISABLED: • 画像の送信：視覚的な説明が必要なときは「画像で説明して」と伝えてください
// DISABLED: 
// DISABLED: ■ 注意点
// DISABLED: • 医学的診断はできません
// DISABLED: • あくまで情報提供や一般的なアドバイスが中心です
// DISABLED: • 専門家への相談も並行して検討してください
// DISABLED: 
// DISABLED: 何か具体的に知りたいことがあれば、お気軽に質問してください。
// DISABLED: `;
// DISABLED:       
// DISABLED:       return ASDSupportManual;
// DISABLED:     }
// DISABLED:     
// DISABLED:     // システムプロンプトを準備（カスタムプロンプトまたはモードに応じたプロンプト）
// DISABLED:     const systemPrompt = customSystemPrompt || getSystemPromptForMode(mode);
// DISABLED:     console.log(`🤖 → システムプロンプト: ${systemPrompt.substring(0, 100)}...`);
// DISABLED:     
    // 会話履歴からメッセージ配列を構築
    const messages = [
      { role: "system", content: systemPrompt }
