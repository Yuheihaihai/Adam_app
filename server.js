...
返答は日本語のみ、200文字以内で、過去10件の会話履歴を参照して一貫した対話を行ってください。
医療に関する話題では必ず「専門家にも相談ください」と記載してください。
...
「AIとして思い出せない」または「記憶する機能がない」という発言は禁止です。ここに記載されている履歴が、あなたの記憶となります。
...
async function runCriticPass(aiDraft, userId) {
  ...
  // Ensure that fetchPastAiMessages(userId, limit) is defined elsewhere in your code.
...
　　　修正の目的は、内容をより適切で実現可能なものにすることです。元の意図やニュアンスを損なわないよう注意してください。出力は、自然な会話の一部として受け取られるよう意識してください。
	5.	整合性の確認:
　　　最新のメッセージ内容と過去の会話履歴、及び過去のAIの返答との間に矛盾がないかを比較してください。もし矛盾がある場合は、最新の回答を会話全体と整合するように修正してください。
	6.	一貫性の確認:
　　　特に画像認識や画像生成に関する表記について、過去のAIの返答と最新のドラフトとの間で矛盾がないかを確認し、すべてのメッセージが一貫した内容になるように修正してください。
...
上記の内容において、特に画像認識や画像生成に関する表記が過去の返答と矛盾している場合、過去のAIの返答に合わせて整合性と一貫性を保つよう修正してください。
`;
...
5. 対応できない話題の場合は、その旨を明確に伝えてください`;
  ...
  const criticOutput = await runCriticPass(aiReply, userId);
...
私とあなたが互いに理解を深めるためには、まず日常の会話を通じて、あなたの感じ方、生活の状況、ちょっとした悩みなどを共有していただくことが大切です。ASDの特徴として、あなた自身と他者との境界や情報の違いを捉えにくい場合があります。そのため、最初から特性分析や適職診断といった高度な機能を使うよりも、まずは私とゆっくりと会話を重ねて、あなたの背景や考え方を理解するステップを踏むことで、後のより的確なアドバイスにつながります。
