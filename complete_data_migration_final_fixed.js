// complete_data_migration_final_fixed.js
// 修正版：AirtableからPostgreSQLへの完全データ移行
...
class CompleteMigrationFinalFixed {
  constructor() {
    // Airtable接続
   ...
   */
  async execute() {
    console.log('🚀 === FIXED COMPREHENSIVE AIRTABLE TO POSTGRESQL MIGRATION ===\n');
  ...
  /**
   * ConversationHistory移行（制約エラー修正版）
   */
  async migrateConversationHistory() {
    console.log('📧 === ConversationHistory Migration (Fixed) ===');
    ...
    try {
      ...
      // Airtableから全データを取得（バッチサイズ制限）
      console.log('   Airtableデータ取得中...');
      const records = await this.getAllRecordsLimited('ConversationHistory', 1000);
      console.log(`   Airtable件数: ${records.length}件`);
  ...
  /**
   * UserAnalysis移行（制約エラー修正版）
   */
  async migrateUserAnalysis() {
    console.log('🤖 === UserAnalysis Migration (Fixed) ===');
    ...
    try {
      ...
      // Airtableから全データを取得（制限付き）
      const records = await this.getAllRecordsLimited('UserAnalysis', 500);
      console.log(`   Airtable件数: ${records.length}件`);
      ...
      for (const record of records) {
        ...
        try {
          ...
          // 存在確認後に挿入
          const existingCheck = await db.pool.query(
            'SELECT id FROM user_ml_analysis WHERE airtable_record_id = $1',
            [record.id]
          );
  ...
  /**
   * JobAnalysis移行（PostgreSQL構文修正版）
   */
  async migrateJobAnalysis() {
    console.log('💼 === JobAnalysis Migration (Fixed) ===');
    
    try {
      // テーブル作成（修正版）
      await this.createJobAnalysisTableFixed();

      // Airtableから全データを取得
      const records = await this.getAllRecordsLimited('JobAnalysis', 100);
      console.log(`   Airtable件数: ${records.length}件`);
      ...
      for (const record of records) {
        ...
        try {
          ...
          // 存在確認後に挿入
          const existingCheck = await db.pool.query(
            'SELECT id FROM job_analysis WHERE airtable_record_id = $1',
            [record.id]
          );
  ...
  /**
   * JobAnalysisテーブル作成（修正版）
   */
  async createJobAnalysisTableFixed() {
    try {
      await db.pool.query(`
      ...
      // インデックス作成
      await db.pool.query(`
        CREATE INDEX IF NOT EXISTS idx_job_analysis_user_id_hash ON job_analysis(user_id_hash)
      `);
      await db.pool.query(`
  ...
  /**
   * Airtableから制限付きでレコードを取得
   */
  async getAllRecordsLimited(tableName, maxRecords = 1000) {
    const records = [];
    let recordCount = 0;
  ...
  displayFinalStats(startTime) {
    ...
    console.log('📊 === FINAL MIGRATION STATISTICS (FIXED) ===');
    ...
    console.log(`🎉 === 修正版移行完了: 合計 ${totalMigrated}件の新しいデータをPostgreSQLに移行 ===`);
  }
}

// スクリプト実行
if (require.main === module) {
  const migration = new CompleteMigrationFinalFixed();
  migration.execute()
    .then(() => {
      console.log('\n✅ 修正版データ移行が正常に完了しました！');
      process.exit(0);
...
module.exports = CompleteMigrationFinalFixed; 